...
{
  bitWrite(data[0], 7, erro_dht_int ? 1 : 0);
  bitWrite(data[0], 6, erro_dht_ext ? 1 : 0);
  bitWrite(data[0], 5, erro_termopar ? 1 : 0);
  bitWrite(data[0], 4, erro_altura ? 1 : 0);
  bitWrite(data[0], 3, erro_rs485 ? 1 : 0);
  bitWrite(data[0], 2, erro_downlink ? 1 : 0);
  bitWrite(data[0], 0, ligarVent ? 1 : 0);
...
void EnviaRS485() {
  ...
  {
    int x = ChecaVentiladores();
    debugPrintLn("Código ventiladores: " + String(x));
    switch (x)
    {
